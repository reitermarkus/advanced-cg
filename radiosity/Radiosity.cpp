...
void calculateFormFactors(const int divisions, const int mc_sample) {
  ...
  for (auto &tri_a : tris) {
    form_factor[&tri_a] = vector<map<Triangle*, vector<double>>>(tri_a.patches.size());
    for (size_t p = 0; p < tri_a.patches.size(); p++) {
      form_factor[&tri_a][p] = map<Triangle*, vector<double>>();
      for (auto &tri_b : tris) {
        form_factor[&tri_a][p][&tri_b] = vector<double>(tri_b.patches.size());
  ...
  for (auto &tri : tris) {
    patch_area[&tri] = vector<double>(tri.patches.size());
    const auto area = tri.area / tri.patches.size();
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (unsigned long p_i = 0; p_i < tris[i].patches.size(); p_i++) {
      /* Loop over all triangles in scene for triangles i */
      for (int j = i + 1; j < n; j++) {
        /* Loop over all patches in rectangle j */
        for (unsigned long p_j = 0; p_j < tris[j].patches.size(); p_j++) {
          double F = 0;
          ...
          for (auto ii = 0; ii < mc_sample; ii++) {
            PatchTriangle t_i = tris[i].patches[p_i];
            PatchTriangle t_j = tris[j].patches[p_j];
  ...
  for (auto &tri_a : tris) {
    for (size_t p_a = 0; p_a < tri_a.patches.size(); p_a++) {
      for (auto &tri_b : tris) {
        for (size_t p_b = 0; p_b < tri_b.patches.size(); p_b++) {
...
unordered_map<Vector, unordered_map<Vector, Color>> calculateVertexColors() {
  ...
  for (auto &tri : tris) {
    for (size_t p = 0; p < tri.patches.size(); p++) {
      vertex_colors.at(tri.normal).emplace(tri.patches[p].a, Color(0, 0, 0));
      vertex_colors.at(tri.normal).emplace(tri.patches[p].b, Color(0, 0, 0));
      vertex_colors.at(tri.normal).emplace(tri.patches[p].c, Color(0, 0, 0));
      vertex_counts.at(tri.normal).emplace(tri.patches[p].a, 0);
      vertex_counts.at(tri.normal).emplace(tri.patches[p].b, 0);
      vertex_counts.at(tri.normal).emplace(tri.patches[p].c, 0);
    }
  }

  for (auto &tri : tris) {
    for (size_t p = 0; p < tri.patches.size(); p++) {
      vertex_counts.at(tri.normal).at(tri.patches[p].a) += 1;
      vertex_counts.at(tri.normal).at(tri.patches[p].b) += 1;
      vertex_counts.at(tri.normal).at(tri.patches[p].c) += 1;
    }
  }

  for (auto &tri : tris) {
    for (size_t p = 0; p < tri.patches.size(); p++) {
      vertex_colors.at(tri.normal).at(tri.patches[p].a) += tri.patches[p].color;
      vertex_colors.at(tri.normal).at(tri.patches[p].b) += tri.patches[p].color;
      vertex_colors.at(tri.normal).at(tri.patches[p].c) += tri.patches[p].color;
    }
  }
...
void calculateRadiosity() {
  for (auto &tri_a : tris) {
    #pragma omp parallel for
    for (unsigned long p_a = 0; p_a < tri_a.patches.size(); p_a++) {
      Color B;

      for (auto &tri_b : tris) {
        for (unsigned long p_b = 0; p_b < tri_b.patches.size(); p_b++) {
          const double Fij = form_factor[&tri_a][p_a][&tri_b][p_b];

          /* Add form factor multiplied with radiosity of previous step */
          if (Fij > 0.0)
            B = B + Fij * tri_b.patches[p_b].color;
        }
      }
...
static int findPatchIndex(const Triangle &triangle, const Ray &ray) {
  for (unsigned long p = 0; p < triangle.patches.size(); p++) {
    if (triangle.patches[p].intersect(ray)) {
      return p;
    }
  }
...
pair<Color, Color> radiance(const Ray &ray, unordered_map<Vector, unordered_map<Vector, Color>> &vertex_colors) {
  ...
  Vector bary = obj.patches[idx].barycentricCoordinatesAt(hitpoint);

  Color a = vertex_colors[obj.normal][obj.patches[idx].a];
  Color b = vertex_colors[obj.normal][obj.patches[idx].b];
  Color c = vertex_colors[obj.normal][obj.patches[idx].c];

  Color interpolated = a * bary.x + b * bary.z + c * bary.y;

  return make_pair(obj.patches[idx].color, interpolated);
}
